...
(() => {
  ...
  function load(){
    try{
      ...
      return {people:clone(defaultPeople), config:clone(defaultConfig), manual:{}, proposal:null};
    }
  }
    ...
    cal.innerHTML = `<div class="month-grid">${
      days.map(date=>{
        ...
        return `<section class="month-day ${out ? "out" : ""}">
          ...
          ${rendered || `<div class="notice">Sin turnos</div>`}
        </section>`;
      }).join("")
    }</div>`;
  }
  ...
  function saveVariables(e){
    ...
    save(); renderVariables(); toast("Variables guardadas");
  }
  ...
  function clearCalendar(){
    if(!confirm("Borrar calendario completo? Se eliminarán cambios manuales y propuestas. Las variables y personas se mantienen.")) return;
    state.manual = {};
    state.proposal = null;
    save();
    renderAll();
  ...
  function applyVariables(){
    ...
    save();
    renderAll();
    toast("Variables aplicadas al calendario");
  }
  ...
  function bind(){
    ...
    $("#resetRules")?.addEventListener("click",()=>{ state.config = clone(defaultConfig); save(); renderVariables(); toast("Reglas iniciales restauradas"); });
    $("#customVarForm")?.addEventListener("submit",e=>{
      ...
      save(); f.reset(); renderVariables(); toast("Variable añadida");
    });
